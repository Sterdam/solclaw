...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // Build cancel_invoice instruction
    const cancelInvoiceIx = await (program.methods as any)
      .cancelInvoice()
      .accounts({
        invoice: invoicePDA,
    ...
    const transaction = new Transaction().add(cancelInvoiceIx);
    ...
    return jsonResponse({
      ...
      message: `Sign and submit to cancel invoice #${invoiceId}`,
    });
  } catch (error: any) {
